...
def get_esp():
    ...
    return ESP_PATH if os.path.exists(ESP_PATH) else None
...
def grub_boot(esp):
    ...
    for root, _, files in os.walk(esp):
        for n in files:
            if n.startswith('grub') and n.endswith('.efi'):
...
def UEFI():
    ...
    bootx64 = os.path.join(esp, ESP_BOOT)
